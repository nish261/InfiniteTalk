...
AUDIO_ENCODER = None
WAV2VEC_FE = None
LOAD_ERROR = None  # If model load failed, store the error here


def load_models():
    global PIPELINE, AUDIO_ENCODER, WAV2VEC_FE, LOAD_ERROR
    if PIPELINE is not None:
        return None  # already loaded
    if LOAD_ERROR is not None:
        return LOAD_ERROR  # already failed
...
def loudness_norm(audio_array, sr=16000, target=-23.0):
    try:
        ...
        return pyln.normalize.loudness(audio_array, loudness, target)
    except Exception:
        return audio_array


def handler(job):
    # Load models on first job (lazy)
    err = load_models()
    if err:
        return {"error": err}
    ...
    try:
        ...
        # ── Download audio ────────────────────────────────────────────────
        raw_audio = f"{tmp}/input_raw.wav"
        if "wav_url" in inp:
            r = subprocess.run(
                ["wget", "-q", "--timeout=60", "-O", raw_audio, inp["wav_url"]],
                capture_output=True, timeout=90
            )
            if r.returncode != 0:
        ...
        subprocess.run(
            ["ffmpeg", "-y", "-i", raw_audio, "-ar", "16000", "-ac", "1", wav_16k],
            capture_output=True, check=True
        )

        # ── Audio embedding ───────────────────────────────────────────────
        speech_array, _ = librosa.load(wav_16k, sr=16000)
        speech_array = loudness_norm(speech_array).astype('float32')

        logger.info(f"Computing audio embedding ({len(speech_array)/16000:.1f}s audio)...")
        ...
        logger.info(f"Generating: steps={sampling_steps} seed={seed} max_frames={max_frames}")
        ...
        if not os.path.exists(out_path) or os.path.getsize(out_path) < 1024:
            return {"error": "Output video missing or empty after generation"}

        logger.info(f"Done: {os.path.getsize(out_path)} bytes")
